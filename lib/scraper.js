...
export async function getJobListings(html){ //This is specifically for indeed.com
 ...
 export async function getJobListingsMonster(html){ //This is specifically for indeed.com
    const $ = cheerio.load(html);
    const jobTitleM = $('.flex-row .summary .title').text() //grabing the job titles nested in 3 classes
    const jobCompanyM = $('.flex-row .summary .company .name').text()
    const jobLocationM = $('.flex-row .summary .location .name').text()
    const allInfoM = [jobTitleM, jobCompanyM, jobLocationM]
    console.log(allInfoM) 
 } 
